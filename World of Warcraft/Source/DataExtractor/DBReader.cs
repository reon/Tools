...
namespace DataExtractor
{
    class DBHeader
    {
        ...
        public Constants.FileFlags MetaTypes   { get; set; }
        ...
        public static DataTable Read(MemoryStream dbStream, Type type)
        {
            ...
            try
            {
                ...
                if (header.IsValidDb4File)
                {
                    ...
                    if (header.IsValidDb4File)
                        header.MetaTypes = (Constants.FileFlags)dbReader.Read<int>();

                    var dataSize = header.RecordCount * header.RecordSize;
                    var indexDataSize = header.RecordCount * 4;
                    var indexDataStart = 0;

                    hasDataOffsetBlock = header.MetaTypes.HasFlag(FileFlags.DataOffset);
                    ...
                    hasIndex = header.MetaTypes.HasFlag(FileFlags.Index);

                    if (!header.MetaTypes.HasFlag(FileFlags.DataOffset))
                        header.StringData = dbReader.ReadBytes((int)header.BlockValue);

                    // Some index data stuff?!
                    if (header.MetaTypes.HasFlag(FileFlags.Unknown))
